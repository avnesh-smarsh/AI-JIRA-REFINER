...
exports.RefinementAgent = void 0;
class RefinementAgent {
    ...
    async processTicket(ticketId) {
        ...
        const ticketResult = await this.mcp.callTool(server, "get_issue", { issueId: ticketId });
        const ticketData = JSON.parse(ticketResult.content[0].text);
        log += `Found Ticket: "${ticketData.summary}"\n`;
        // 2. Extract Keywords for Search
        const keywords = ticketData.summary.split(" ").filter((w) => w.length > 3);
    ...
    generateMockLLMResponse(ticket, repos, log) {
        return `
...
* **Repositories:** ${repos.map((r) => `  * [${r.name}](${r.url}) - *${r.description}*`).join("\n")}
...
exports.RefinementAgent = RefinementAgent;
